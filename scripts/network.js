if (networkEnabled) {
  var getBlockCount = function() {
    ...
    request.open('GET', "https://mywallet.crownplatform.com/blockcount", true);  
  ...
  var getUnspentTransactions = function () {
    var request = new XMLHttpRequest()
    request.open('GET', "https://mywallet.crownplatform.com/utxo/" + publicKeyForNetwork, true)
    request.onload = function () {
  ...
  var sendTransaction = function (hex, msg = '') {
    var request = new XMLHttpRequest();
    request.open('GET', 'https://mywallet.crownplatform.com/sendtx/' + hex, true)
    request.onerror = function () {
